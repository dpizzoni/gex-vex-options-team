...
def get_options(symbol: str = Query(..., description="Ticker symbol e.g. SPY"), expiration: str = Query(None, description="Expiration date YYYY-MM-DD")):
    try:
        ...
        # R8 fix: DST-aware close time — Apr–Oct = EDT (UTC-4) → 20:00 UTC; Nov–Mar = EST (UTC-5) → 21:00 UTC.
        month = int(selected_exp[5:7])
        close_utc = "T20:00:00" if 4 <= month <= 10 else "T21:00:00"
        exp_dt = datetime.strptime(selected_exp + close_utc, "%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone.utc)
