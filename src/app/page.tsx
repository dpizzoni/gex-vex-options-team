...
export default function Home() {
  ...
  const rows = React.useMemo(() => {
    ...
    const computedRows = baseRows.map(row => {
      ...
      let vanna = 0;
      let vex = 0;
      const sigma = sanitizeIV(row.iv); // R5: centralizado en engine [0.03, 4.0]
      const ivNormalized = sigma;

      if (sigma > 0 && data.spot > 0) {
        vanna = calcVanna(data.spot, row.strike, T, sigma);
        // R3: convención call+ / put- idéntica a GEX (dealer largo calls, corto puts)
        vex = (row.type === 'CALL' ? 1 : -1) * vanna * row.oi * data.spot;
  ...
  const { netVex, vannaFlip, medianVanna } = React.useMemo(() => {
    ...
    allOptions.forEach(opt => {
      const sigma = sanitizeIV(opt.impliedVolatility); // R5

      let vex = 0;
      if (sigma > 0 && data.spot > 0) {
        const vanna = calcVanna(data.spot, opt.strike, T, sigma);
        // R3: call+ / put- para consistencia con GEX
        vex = (opt.type === 'CALL' ? 1 : -1) * vanna * opt.openInterest * data.spot;
  ...
  const tableMatrixData = useMemo(() => {
    ...
    // isVex=true: RAW aplica tipo+/-, DEALER omite abs (vanna tiene signo intrínseco vía d2) — R2+R3
    const getExposureValue = (contract: any, baseValue: number, isVex = false) => {
      if (matrixAggregation === "RAW") {
        return isVex ? (contract.type === "CALL" ? 1 : -1) * baseValue : baseValue;
      }
      if (matrixAggregation === "DEALER") {
        ...
        return isVex ? -clientBias * baseValue : -clientBias * Math.abs(baseValue);
      }
      return baseValue;
    };
      ...
      result = result.map(chain => {
        ...
        const processOpts = (opts: any[], type: "CALL" | "PUT") => opts.map(opt => {
          ...
          if (sigma > 0 && data.spot > 0) {
            ...
            vexVal = rawVanna * opt.openInterest * data.spot; // raw unsigned — sign via getExposureValue(isVex=true)
          }
          const optWithType = { ...opt, type, expiration: chain.expiration };
          return {
            ...optWithType,
            gex: getExposureValue(optWithType, opt.gex || 0),
            vanna: getExposureValue(optWithType, vexVal, true) // R2+R3: isVex=true
          };
        });
  ...
  const analysisSnapshot = useMemo(() => {
    ...
    // Calculate VEX exposure — R2+R3+R5: sanitizeIV + isVex=true para signo correcto
    const calculateVex = (opt: any, type: "CALL" | "PUT") => {
      let vexVal = 0;
      const sigma = sanitizeIV(opt.impliedVolatility); // R5
      if (sigma > 0 && spot > 0) {
        const vanna = calcVanna(spot, opt.strike, T, sigma);
        vexVal = vanna * opt.openInterest * spot; // raw unsigned — sign via getExposureValue(isVex=true)
      }
      return getExposureValue({ ...opt, type, expiration: activeExp }, vexVal, true); // R2+R3
    };
